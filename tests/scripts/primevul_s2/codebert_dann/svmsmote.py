...
THRESHOLD            = 0.5
RESUME               = True
METHOD_VERSION       = "dann_xgboost_svmsmote_v1"

SVMSMOTE_K_NEIGHBORS_DEFAULT = 5    
SVMSMOTE_M_NEIGHBORS_DEFAULT = 10   
...
def apply_svmsmote(X_source, y_source, seed):
    
    classes, counts = np.unique(y_source, return_counts=True)
    minority_count = int(counts.min())

    if minority_count < 2:
        print(f"      [SVMSMOTE] Minority class has only {minority_count} sample(s) -- "
              f"skipping SVMSMOTE, returning original data unchanged.")
        return X_source, y_source

    k_neighbors = min(SVMSMOTE_K_NEIGHBORS_DEFAULT, minority_count - 1)
    m_neighbors = min(SVMSMOTE_M_NEIGHBORS_DEFAULT, minority_count - 1)
    if k_neighbors < SVMSMOTE_K_NEIGHBORS_DEFAULT or m_neighbors < SVMSMOTE_M_NEIGHBORS_DEFAULT:
        print(f"      [SVMSMOTE] Minority class has only {minority_count} samples -- "
              f"lowering k_neighbors to {k_neighbors} and m_neighbors to {m_neighbors}.")

    svmsmote = SVMSMOTE(random_state=seed, k_neighbors=k_neighbors, m_neighbors=m_neighbors)
    try:
        X_resampled, y_resampled = svmsmote.fit_resample(X_source, y_source)
    except (ValueError, RuntimeError) as exc:
        print(f"      [SVMSMOTE] fit_resample failed ({exc}) -- returning original data unchanged.")
        return X_source, y_source

    X_resampled = X_resampled.astype(np.float32)
    y_resampled = y_resampled.astype(np.int32)

    print(f"      [SVMSMOTE] Source pool: {X_source.shape[0]} -> {X_resampled.shape[0]} samples "
...
def train_dann(x_source, y_source, x_target, seed):
    ...
    classification_loss_fn = nn.BCEWithLogitsLoss()   # no pos_weight -- SVMSMOTE already balances classes
...
def save_confusion_matrix(metrics, project, path):
    ...
    ax.set_title(f"Confusion Matrix ({project}) - DANN + XGBoost + SVMSMOTE")
...
# =========================================================
def run_single(project, run_number, X_source_svmsmote, y_source_svmsmote, X_target, y_target,
    ...
    feature_extractor, classifier, discriminator, history = train_dann(
        X_source_svmsmote, y_source_svmsmote, X_target, seed
    )

    train_features = extract_features(feature_extractor, X_source_svmsmote)
    target_features = extract_features(feature_extractor, X_target)

    train_probs = classifier_probabilities(feature_extractor, classifier, X_source_svmsmote)
    sample_weights = np.where(y_source_svmsmote == 1, train_probs, 1 - train_probs)

    model_xgb = train_xgboost(train_features, y_source_svmsmote, sample_weights, seed)
    ...
    result = {
        ...
        "oversampling_method": "SVMSMOTE",
        "project": project,
        "run": run_number,
        "seed": seed,
        "final_classifier": "XGBoost on DANN-extracted features",
        "n_source_samples_svmsmote": int(len(y_source_svmsmote)),
        "n_target_samples": int(len(y_target)),
        "n_source_vulnerable_svmsmote": int(y_source_svmsmote.sum()),
...
def run_project(project, X_all, y_all, projects_all, ids_all,
    ...
    print(f"Source (pre-SVMSMOTE): {X_source.shape} vulnerable={int(y_source.sum())} benign={int((y_source == 0).sum())}")
    print(f"Target: {X_target.shape} vulnerable={int(y_target.sum())} benign={int((y_target == 0).sum())}")
    print("=" * 88)

    X_source_svmsmote, y_source_svmsmote = apply_svmsmote(X_source, y_source, seed=SEED)
    ...
    for run_number in range(1, NUM_RUNS + 1):
        ...
        try:
            result = run_single(
                project, run_number, X_source_svmsmote, y_source_svmsmote, X_target, y_target,
                target_ids, scaler, output_dir
            )
...
def main():
    ...
    master_path = output_root / "dann_xgboost_svmsmote_all_projects.json"
