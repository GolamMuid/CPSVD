...
def apply_smote(X_source, y_source, seed):
    ...
    if minority_count < 2:
        print(f"      [SMOTE] Minority class has only {minority_count} sample(s) -- "
              f"skipping SMOTE, returning original data unchanged.")
        return X_source, y_source

    k_neighbors = min(SMOTE_K_NEIGHBORS_DEFAULT, minority_count - 1)
    if k_neighbors < SMOTE_K_NEIGHBORS_DEFAULT:
        print(f"      [SMOTE] Minority class has only {minority_count} samples -- "
              f"lowering k_neighbors from {SMOTE_K_NEIGHBORS_DEFAULT} to {k_neighbors}.")

    smote = SMOTE(random_state=seed, k_neighbors=k_neighbors)
    X_resampled, y_resampled = smote.fit_resample(X_source, y_source)
    X_resampled = X_resampled.astype(np.float32)
    y_resampled = y_resampled.astype(np.int32)

    print(f"      [SMOTE] Source pool: {X_source.shape[0]} -> {X_resampled.shape[0]} samples "
...
def train_dann(x_source, y_source, x_target, seed):
    ...
    classification_loss_fn = nn.BCEWithLogitsLoss()   # no pos_weight -- SMOTE already balances classes
...
def save_confusion_matrix(metrics, project, path):
    ...
    ax.set_title(f"Confusion Matrix ({project}) - DANN + XGBoost + SMOTE")
...
# =========================================================
def run_single(project, run_number, X_source_smote, y_source_smote, X_target, y_target,
    ...
    feature_extractor, classifier, discriminator, history = train_dann(
        X_source_smote, y_source_smote, X_target, seed
    )

    # Extract domain-invariant features for train + target
    train_features = extract_features(feature_extractor, X_source_smote)
    ...
    train_probs = classifier_probabilities(feature_extractor, classifier, X_source_smote)
    sample_weights = np.where(y_source_smote == 1, train_probs, 1 - train_probs)

    model_xgb = train_xgboost(train_features, y_source_smote, sample_weights, seed)
    ...
    result = {
        ...
        "n_source_samples_smote": int(len(y_source_smote)),
        "n_target_samples": int(len(y_target)),
        "n_source_vulnerable_smote": int(y_source_smote.sum()),
...
def run_project(project, X_all, y_all, projects_all, ids_all,
    ...
    print(f"Source (pre-SMOTE): {X_source.shape} vulnerable={int(y_source.sum())} benign={int((y_source == 0).sum())}")
    print(f"Target: {X_target.shape} vulnerable={int(y_target.sum())} benign={int((y_target == 0).sum())}")
    print("=" * 88)

    # SMOTE applied ONCE per project, on the full source pool, shared across all runs
    X_source_smote, y_source_smote = apply_smote(X_source, y_source, seed=SEED)
    ...
    for run_number in range(1, NUM_RUNS + 1):
        ...
        try:
            result = run_single(
                project, run_number, X_source_smote, y_source_smote, X_target, y_target,
                target_ids, scaler, output_dir
            )
...
def main():
    ...
    master_path = output_root / "dann_xgboost_smote_all_projects.json"
