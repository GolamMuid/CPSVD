...
EMB_KEY         = "emb"
OUTPUT_ROOT     = "results/no_oversampling"
...
def save_confusion_matrix(metrics, project, path):
    ...
    ax.set_title(f"Confusion Matrix ({project}) - DANN + XGBoost")
...
# =========================================================
def run_single(project, run_number, X_source, y_source, X_target, y_target,
                target_ids, scaler, class_weight_dict, output_dir):
...
def run_project(project, X_all, y_all, projects_all, ids_all,
    ...
    # Class weights computed from the FULL source pool (before any split)
    ...
    print(f"Source: {X_source.shape} vulnerable={int(y_source.sum())} benign={int((y_source == 0).sum())}")
    print(f"Target: {X_target.shape} vulnerable={int(y_target.sum())} benign={int((y_target == 0).sum())}")
    print(f"Class weights (full source pool): {class_weight_dict}")
    print("=" * 88)
    ...
    for run_number in range(1, NUM_RUNS + 1):
        ...
        try:
            result = run_single(
                project, run_number, X_source, y_source, X_target, y_target,
                target_ids, scaler, class_weight_dict, output_dir
            )
            ...
            atomic_json_dump(state, state_path)
            continue  # keep going with remaining runs for this project
...
def main():
    ...
    master_path = output_root / "dann_xgboost_all_projects.json"
    ...
    for project_index, project in enumerate(TEST_PROJS, start=1):
        ...
        try:
            ...
            continue  # move on to the next project regardless of this failure
